       ...
       $status   = remove_junk($db->escape($_POST['status']));
         ...
         $result = $db->query($sql);
          if($result && $db->affected_rows() === 1){
            $session->msg('s',"Conta atualizada ");
            redirect('edit_user.php?id='.(int)$e_user['id'], false);
          } else {
            $session->msg('d',' Erro ao atualizar');
            redirect('edit_user.php?id='.(int)$e_user['id'], false);
          }
    } else {
...
if(isset($_POST['update-pass'])) {
  ...
  if(empty($errors)){
     ...
     $h_pass   = sha1($password);
       ...
       $result = $db->query($sql);
        if($result && $db->affected_rows() === 1){
          $session->msg('s',"Senha de usuário atualizada! ");
          redirect('edit_user.php?id='.(int)$e_user['id'], false);
        } else {
          $session->msg('d',' Erro ao atualizar senha');
          redirect('edit_user.php?id='.(int)$e_user['id'], false);
        }
  } else {
    $session->msg("d", $errors);
     ...
     <div class="panel panel-default">
       <div class="panel-heading">
        <strong>
          ...
          Atualizar <?php echo remove_junk(ucwords($e_user['name'])); ?>
        </strong>
       </div>
            ...
            <div class="form-group">
              <label for="status">Status</label>
                <select class="form-control" name="status">
                  ...
                  <option <?php if($e_user['status'] === '0') echo 'selected="selected"';?> value="0">Desativado</option>
                </select>
            </div>
  ...
  <div class="col-md-6">
    <div class="panel panel-default">
      <div class="panel-heading">
        <strong>
          <span class="glyphicon glyphicon-th"></span>
          Mudar senha de <?php echo remove_junk(ucwords($e_user['name'])); ?>
        </strong>
      </div>
      <div class="panel-body">
        <form action="edit_user.php?id=<?php echo (int)$e_user['id'];?>" method="post" class="clearfix">
          ...
          <div class="form-group clearfix">
                  <button type="submit" name="update-pass" class="btn btn-danger pull-right">Atualizar</button>
          </div>
        </form>
      </div>
    </div>
